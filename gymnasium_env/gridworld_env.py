...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, size=30, render_mode=None, max_steps=None):
        ...
        # Start & goal like the figure (top-left → bottom-right)
        self.start_pos = (0, 0)
        self.goal_pos = (self.size - 1, self.size - 1)

        self.steps = 0
        self.max_steps = max_steps if max_steps is not None else 600  # fast cap

        # build static obstacles (black cells)
    ...
    def _static_obstacles_like_figure(self):
        ...
        Layout is an 'organized' maze-like set of blocks similar to the figure:
        - few big blocks
        - some columns/rows barriers
        - ensures a path exists from start(0,0) to goal(29,29)
        """
        obs = set()

        # Top-left big block (like black region)
        self._add_rect(obs, 1, 5, 2, 4)

        # Top row scattered blocks
        self._add_rect(obs, 0, 2, 7, 8)
        self._add_rect(obs, 0, 2, 12, 13)
        self._add_rect(obs, 1, 3, 16, 17)

        # Left-middle vertical wall (with gaps)
        ...
        # Add a horizontal barrier with gaps (organized)
        for c in range(0, self.size):
            if c not in (5, 6, 14, 15, 27):  # gaps
                obs.add((16, c))

        # Make sure start corridor is not blocked
        obs.discard((0, 1))
        obs.discard((1, 0))
        obs.discard((1, 1))

        # Make sure goal corridor is not blocked
    ...
    def step(self, action):
        ...
        # Reward shaping (helps learn faster)
        if terminated:
            reward = 150.0
        else:
            reward = -0.1
